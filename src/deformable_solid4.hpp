    ...
    for (int i=0; i<this->pl.size(); i++) {
      ...
      for (int j=0; j<this->next_part[i].size(); j++) {
        ...
        if (dist > SPRING_DAMPER_THRE) {
          ...
          //   * this->tmp_d;
          this->pl[i]->deform_force += K_SPRING * (dist - this->length) * this->tmp_d;
          this->pl[i]->deform_force += K_DAMPER * (this->pl[nid]->vel - this->pl[i]->vel).dot(this->tmp_d)
            * this->tmp_d;
        }
      }
    }
