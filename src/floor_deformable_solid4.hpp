  ...
  void floor_collision_penalty(boost::shared_ptr<particle>& part, bool apply_grav=true) {
    ...
    for (int k=0; k<this->floors.size(); k++) {
      ...
      if (dist_to_floor < 0) {
        ...
        // Divide force to rigid force and deformable force
        // this->pl[i]->force = (1 - DEFORM_RATIO) * this->pl[i]->force;
        // this->pl[i]->deform_force = DEFORM_RATIO * this->pl[i]->force;
        part->force = (1 - DEFORM_RATIO) * part->force;
        part->deform_force = DEFORM_RATIO * part->force;
      }
    }
  }
